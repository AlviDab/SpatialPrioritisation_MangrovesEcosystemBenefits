...
library(exactextractr)
  ...
  PUs_carbon <- PUs %>%
    slice(1:9110) %>% 
    st_transform(crs = crs(s[[1]])) %>% #I transform the crs of the PUs so that have the same crs of the rasters
    st_make_valid()

  #Function to extract the carbon data for each PUs
  carb_data_list <- future_lapply(s, function(carbon_layer) {
                             a <- exact_extract(carbon_layer, PUs_carbon, c('mean', 'count')) #I extract all the values that intercept with the PUs
                             }
                           )

  final_carbon <- carb_data_list %>% 
    future_lapply(function(x) {
      x %>% 
        mutate(ID = PUs_carbon$ID) %>% 
        filter(mean > 0)
      })
