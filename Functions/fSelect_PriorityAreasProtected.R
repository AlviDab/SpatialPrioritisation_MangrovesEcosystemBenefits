...
f_PriorityAreasProtected <- function(result, n, IUCN = "I-IV") {
  ...
  if(IUCN == "All") {  
    ...
    # Area of priority mangroves outside of Protected_I_VI areas
    priority_Protected_I_VI <- result %>%
      as_tibble() %>% 
      filter(rank <= n) %>% #filter all PUs <= that priority
      filter(Protected_I_VI == TRUE) %>% #filter all the PUs already Protected_I_VI
      group_by(country) %>% #group by country
      summarise(priority_Protected_I_VI = sum(AreaGMWKm)) #calculate the extent priority areas currently Protected_I_VI
    
    priority_notProtected_I_VI <- result %>%
      as_tibble() %>% 
      filter(rank <= n) %>%  #filter all the PUs <= n
      filter(Protected_I_VI == FALSE) %>% #Keep only not Protected_I_VI PUs
      group_by(country) %>% #group by country
      summarise(priority_notProtected_I_VI = sum(AreaGMWKm)) #Calculate the extent of not Protected_I_VI priority areas
    
    priority_perc_Protected_I_VI_country <- priority_Protected_I_VI %>% 
      full_join(priority_notProtected_I_VI, by = "country") %>% 
      replace(is.na(.), 0) %>% #replace NAs with 0
      mutate(perc_priority_Protected_I_VI = priority_Protected_I_VI/(priority_Protected_I_VI + priority_notProtected_I_VI)*100) #Calculate percentage of priority Protected_I_VI 
    
    #Not by country
    priority_perc_Protected_I_VI_country %>% 
      summarise(across(!country, sum)) %>% #Calculate the amount of priority Protected_I_VI not by country
      mutate(perc_priority_Protected_I_VI = priority_Protected_I_VI/(priority_Protected_I_VI + priority_notProtected_I_VI)*100)
    ...
    priority_perc_Protected_I_VI_region <- priority_perc_Protected_I_VI_country %>% 
      left_join(region_country, by = "country") %>% #left join column of continent and country
      group_by(continent) %>% #group by continent
      summarise(across(!country, sum)) %>% #make sum for all the columns that are not the country column
      mutate(perc_priority_Protected_I_VI = priority_Protected_I_VI/(priority_Protected_I_VI + priority_notProtected_I_VI)*100) #calculate the percentage of priority areas Protected_I_VI by continent
    
    #Not by country
    priority_perc_Protected_I_VI_tot <- priority_perc_Protected_I_VI_country %>%
      summarise(across(!country, sum)) %>% #summarise across all the columns that are not country column
      mutate(perc_priority_Protected_I_VI = priority_Protected_I_VI/(priority_Protected_I_VI + priority_notProtected_I_VI)*100) #total priority areas already Protected_I_VI
    
    priority <- list(priority_perc_Protected_I_VI_country,  priority_perc_Protected_I_VI_region, priority_perc_Protected_I_VI_tot) #list outputs
  }
